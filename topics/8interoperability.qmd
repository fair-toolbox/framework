...
## Inventing a format when a standard exists

A custom format usually solves a narrow problem while creating a much broader one: every downstream tool now needs code written specifically to read it.

## Hard-coding assumptions that block composition

Absolute file paths, interactive prompts, or output written only to a fixed location all make a tool harder to slot into someone else's pipeline, even if the format it reads and writes is otherwise standard.

## Leaving inputs and outputs undocumented

Without a documented schema or metadata describing what a tool expects and produces, other tools—and other people—can only find out by trial and error.
...
- Do you use open, standard formats for input and output data wherever one exists?
- Do your command-line tools or APIs follow standard conventions (stdin/stdout and exit codes, or REST and OpenAPI)?
- Is your software described with machine-readable metadata (for example CodeMeta or a domain ontology)?
- Can your software be used as a step in a workflow manager without a special wrapper?
