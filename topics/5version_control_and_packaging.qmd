...
nocite: |
  ...
  @rsqkit-using-version-control
  @swcarpentry-git-novice
  @turingway-vcs
  @rsqkit-packaging-releasing-software
  @rsqkit-releasing-software
---
...
Versioning and packaging solve this by turning a moving target into something others can reliably install, reference, and build on. This guide assumes you already use version control day-to-day; it focuses specifically on how to turn commits into releases people can depend on.
...
A user who sees a MINOR version bump knows they can upgrade safely. A MAJOR bump tells them to check for breaking changes first. This shared meaning is what makes version numbers useful information rather than an arbitrary counter.
...
On GitHub, pushing a tag can also be turned into a formal Release, with release notes attached. This gives every version a permanent, citable reference—unlike a branch name or commit on `main`, which keeps moving.
...
Most ecosystems have a standard packaging format: `pip`/PyPI for Python, CRAN or Bioconductor for R, npm for JavaScript. The details differ, but the goal is the same: a minimal metadata file describing your package, and a build step that turns your source code into an installable artifact.
...
Publishing a package without a corresponding git tag means there is no way to trace an installed version back to the exact source code that produced it.

## Inconsistent, manual releases

When releases depend on someone remembering the right sequence of manual steps, the process eventually gets done wrong—or skipped entirely under time pressure.

## Breaking changes without a version bump

Changing behaviour without incrementing the MAJOR version misleads anyone relying on semantic versioning to judge whether an upgrade is safe.
...
- Do your releases follow semantic versioning?
- Is every release marked with a git tag?
- Can someone install your software with a single command, rather than cloning and configuring it manually?
- Is your release process automated through CI/CD rather than manual steps?
